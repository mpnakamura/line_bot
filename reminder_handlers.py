...
def confirm_reminder(user_id, user_message):
    ...
    if user_message == "はい":
        if finalize_reminder(reminder_id):
            ...
            logging.info(f"Reminder ID: {reminder_id} saved successfully.")
            return TextSendMessage(text="予定を保存しました。")
        else:
            logging.warning(f"Failed to save Reminder ID: {reminder_id}.")
...
def finalize_reminder(reminder_id):
    ...
    try:
        ...
        logging.error(f"Error checking reminder in the database for Reminder ID: {reminder_id}: {e}")
        return False
    finally:
        conn.close()
